...
namespace Infusio.Http
{
    ...
    public static class HttpSupport
    {
        ...
        static EitherAsync<InfusioError, InfusioResult<T>> RunAsync<T>(InfusioOp<T> op, InfusioState state, InfusioClient client) =>
            ...
            op is InfusioOp<T>.CreateProduct _68 ? Exe(_68, () => client.CreateProduct(_68.CreateProductModel), _68.Next, state, client) :
            op is InfusioOp<T>.ListProductsFromSyncToken _69 ? Exe(_69, () => client.ListProductsFromSyncToken(_69.Offset, _69.Limit, _69.SyncToken), _69.Next, state, client) :
            op is InfusioOp<T>.RetrieveProduct _70 ? Exe(_70, () => client.RetrieveProduct(_70.ProductId), _70.Next, state, client) :
            op is InfusioOp<T>.DeleteProduct _71 ? Exe(_71, () => client.DeleteProduct(_71.ProductId), _71.Next, state, client) :
            op is InfusioOp<T>.CreateProductSubscription _72 ? Exe(_72, () => client.CreateProductSubscription(_72.CreateProductSubscriptionModel, _72.ProductId), _72.Next, state, client) :
            op is InfusioOp<T>.RetrieveProductSubscription _73 ? Exe(_73, () => client.RetrieveProductSubscription(_73.SubscriptionId, _73.ProductId), _73.Next, state, client) :
            op is InfusioOp<T>.DeleteProductSubscription _74 ? Exe(_74, () => client.DeleteProductSubscription(_74.SubscriptionId, _74.ProductId), _74.Next, state, client) :
            op is InfusioOp<T>.GetApplicationEnabled _75 ? Exe(_75, () => client.GetApplicationEnabled(), _75.Next, state, client) :
            op is InfusioOp<T>.GetContactOptionTypes _76 ? Exe(_76, () => client.GetContactOptionTypes(), _76.Next, state, client) :
            op is InfusioOp<T>.RetrieveSubscriptionModel _77 ? Exe(_77, () => client.RetrieveSubscriptionModel(), _77.Next, state, client) :
            op is InfusioOp<T>.ListTags _78 ? Exe(_78, () => client.ListTags(_78.Category, _78.Offset, _78.Limit), _78.Next, state, client) :
            op is InfusioOp<T>.CreateTag _79 ? Exe(_79, () => client.CreateTag(_79.Tag), _79.Next, state, client) :
            op is InfusioOp<T>.CreateTagCategory _80 ? Exe(_80, () => client.CreateTagCategory(_80.TagCategory), _80.Next, state, client) :
            op is InfusioOp<T>.GetTag _81 ? Exe(_81, () => client.GetTag(_81.Id), _81.Next, state, client) :
            op is InfusioOp<T>.ListContactsForTagId _82 ? Exe(_82, () => client.ListContactsForTagId(_82.TagId, _82.Offset, _82.Limit), _82.Next, state, client) :
            op is InfusioOp<T>.ApplyTagToContactIds _83 ? Exe(_83, () => client.ApplyTagToContactIds(_83.Ids, _83.TagId), _83.Next, state, client) :
            op is InfusioOp<T>.RemoveTagFromContactIds _84 ? Exe(_84, () => client.RemoveTagFromContactIds(_84.Ids, _84.TagId), _84.Next, state, client) :
            op is InfusioOp<T>.RemoveTagFromContactId _85 ? Exe(_85, () => client.RemoveTagFromContactId(_85.ContactId, _85.TagId), _85.Next, state, client) :
            op is InfusioOp<T>.ListTasks _86 ? Exe(_86, () => client.ListTasks(_86.Order, _86.Offset, _86.Limit, _86.Completed, _86.Until, _86.Since, _86.UserId, _86.HasDueDate, _86.ContactId), _86.Next, state, client) :
            op is InfusioOp<T>.CreateTask _87 ? Exe(_87, () => client.CreateTask(_87.Task), _87.Next, state, client) :
            op is InfusioOp<T>.RetrieveTaskModel _88 ? Exe(_88, () => client.RetrieveTaskModel(), _88.Next, state, client) :
            op is InfusioOp<T>.ListTasksForCurrentUser _89 ? Exe(_89, () => client.ListTasksForCurrentUser(_89.Order, _89.Offset, _89.Limit, _89.Completed, _89.Until, _89.Since, _89.UserId, _89.HasDueDate, _89.ContactId), _89.Next, state, client) :
            op is InfusioOp<T>.GetTask _90 ? Exe(_90, () => client.GetTask(_90.TaskId), _90.Next, state, client) :
            op is InfusioOp<T>.UpdateTask _91 ? Exe(_91, () => client.UpdateTask(_91.Task, _91.TaskId), _91.Next, state, client) :
            op is InfusioOp<T>.DeleteTask _92 ? Exe(_92, () => client.DeleteTask(_92.TaskId), _92.Next, state, client) :
            op is InfusioOp<T>.UpdatePropertiesOnTask _93 ? Exe(_93, () => client.UpdatePropertiesOnTask(_93.Task, _93.TaskId), _93.Next, state, client) :
            op is InfusioOp<T>.ListTransactions _94 ? Exe(_94, () => client.ListTransactions(_94.ContactId, _94.Offset, _94.Limit, _94.Until, _94.Since), _94.Next, state, client) :
            op is InfusioOp<T>.GetTransaction _95 ? Exe(_95, () => client.GetTransaction(_95.TransactionId), _95.Next, state, client) :
