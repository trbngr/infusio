...
namespace Infusio.Http
{
    ...
    public static class HttpSupport
    {
        ...
        public static Task<Either<InfusioError, InfusioResult<T>>> RunWith<T>(this InfusioOp<T> op, InfusioClient client, InfusioLogging logging = InfusioLogging.WithoutLogs) =>
            RunAsync(op, InfusioState.Create(logging), client).ToEither();
